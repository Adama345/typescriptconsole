...
export async function menuPaiements(utilisateur: Utilisateur) {
    ...
    const groupes = data.groups.filter(g => 
    ...
    const { depenseId } = await inquirer.prompt([
        {
            ...
            choices: depenses.map(d => ({
                name: `${d.nom} - ${d.montant} FCFA (${new Date(d.date).toLocaleDateString()})`,
                value: d.id
            }))
        }
    ]);
...
async function processPaiement(depenseId: number, membreId: number) {
    ...
    const paiements = data.Payer.filter(p => 
        p.depenseId === depense.id && 
        p.membreId === membreId
    );
    ...
    if (resteAPayer <= 0) {
        console.log(chalk.green("Vous avez déjà payé votre part pour cette dépense."));
        return;
    }
    ...
    const { montant, methode } = await inquirer.prompt([
        {
            ...
            validate: (input: unknown) => {  // Utilisez 'unknown' comme type d'entrée
                ...
                if (amount > resteAPayer) return `Le montant ne peut pas dépasser ${resteAPayer.toFixed(2)}`;
                return true;
            },
            filter: (input: unknown) => {  // S'assure que la valeur est convertie en nombre
                return Number(input);
            }
        },
        {
...
export function validerPaiements(chefDeGroupeId: number) {
    ...
    const groupes = data.groups.filter(g => g.chefDeGroupe === chefDeGroupeId);
    const depenses = data.depenses.filter(d => 
        groupes.some(g => g.chefDeGroupe === d.chefDeGroupe)
    );
    
    const paiementsEnAttente = data.Payer.filter(p => 
    ...
    inquirer.prompt([
        ...
        inquirer.prompt([
            ...
            const paiement = data.Payer.find(p => p.id === selection.paiementId);
            if (!paiement) return;
            
            if (answer.action === "valider") {
                paiement.statut = "validé";
                console.log(chalk.green("Paiement validé avec succès!"));
            } else {
