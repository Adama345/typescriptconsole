...
export function genererRapportHebdomadaire(
    ...
    const paiementsSemaine = data.Payer.filter((p) => {
        ...
        return (
            depenseAssociee &&
            depenseAssociee.chefDeGroupe === groupe.chefDeGroupe
        );
    });

    // Calculer les totaux
    const depensesTotal = depensesSemaine.reduce(
        (sum, d) => sum + d.montant,
        0
    );
...
export function afficherRapportHebdomadaire(rapport: RapportHebdomadaire) {
    ...
    console.log(
        `Groupe: ${data.groups.find((g) => g.id === rapport.groupeId)?.nom}`
    );
    console.log("\n--- Totaux ---");
    ...
    rapport.depensesParMembre.forEach((item) => {
        ...
        console.log(
            `${membre?.prenom} ${membre?.nom}: ${item.montant.toFixed(2)}`
        );
    });
